...
def check_mat(m, r, c, verbose=False):
    ...
    if np.any(c[1:] == c[:-1]):
        msg = """mat contains repeated feature names, please make them unique."""
    ...
    if np.any(~np.isfinite(m.data)):
        msg = """mat contains non finite values (nan or inf), please set them to 0 or remove them."""
        raise ValueError(msg)
...
def extract(mat, use_raw=True, verbose=False, dtype=np.float32):
    ...
    if type(mat) is list:
        ...
        msg = """mat must be a list of [matrix, samples, features], dataframe (samples x features) or an AnnData
        instance."""
        raise ValueError(msg)
...
def filt_min_n(c, net, min_n=5):
    ...
    if net.shape[0] == 0:
        msg = f"""No sources with more than min_n={min_n} targets. Make sure mat and net have shared target features or
...
def rename_net(net, source="source", target="target", weight="weight"):
    ...
    if source not in net.columns:
        msg = f"Column name '{source}' not found in net. Please specify a valid column."
        raise pd.errors.InvalidColumnName(msg)
    if target not in net.columns:
        msg = f"Column name '{target}' not found in net. Please specify a valid column."
        raise pd.errors.InvalidColumnName(msg)
    if weight is not None:
        if weight not in net.columns:
            msg = (
                f"Column name '{weight}' not found in net. Please specify a valid column. "
                f"Alternatively, set to `None` if no weights are available."
            )
            raise pd.errors.InvalidColumnName(msg)
    ...
    if is_d > 0:
        msg = "net contains repeated edges, please remove them."
        raise ValueError(msg)
