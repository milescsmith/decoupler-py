...
@nb.njit(
    nb.types.Tuple((nb.f4[:, :], nb.i8[:]))(
        nb.f4[:, :], nb.i8[:, :], nb.f4[:], nb.i8[:], nb.i8
    ),
    cache=True,
)
...
def get_inter_pvals(nes_i, ss_i, sub_net, n_targets):
    ...
    for j in range(sub_net.shape[1]):
        ...
        reg = ((sub_net[trgt_msk] != 0) * sub_net[trgt_msk, j].reshape(-1, 1)).astype(
            np.float32
        )
