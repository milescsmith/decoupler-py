...
def recall(y_true, y_score):
    tp = np.sum((y_score != 0.0) & (y_true != 0.0))
    fn = np.sum((y_score == 0.0) & (y_true != 0.0))
    return tp / (tp + fn)
...
def check_m_inputs(y_true, y_score):
    ...
    lbl = np.array([0, 1])
    if unq.size > 2:
        msg = """Ground truth vector contains more than one
        class. Only binary classes are supported."""
        raise ValueError(msg)
    elif not np.all(unq == lbl):
        msg = """Ground truth binary classes must be 0 and 1."""
        raise ValueError(msg)
...
@nb.njit(
    nb.types.Tuple((nb.f4[:], nb.f4[:, :]))(nb.f4[:], nb.f4[:], nb.i8, nb.i8),
    cache=True,
)
