...
def check_if_skranger():
    try:
        ...
        msg = (
            "sklearn is not installed. Please install it with: pip install scikit-learn"
        )
...
def fit_rf(sr, net, sample, trees=100, min_leaf=5, n_jobs=-1, seed=42):
    # Fit Random Forest
    regr = sr.RandomForestRegressor(
        n_estimators=trees, min_samples_leaf=min_leaf, n_jobs=n_jobs, random_state=seed
    )
    regr.fit(net, sample)
...
def mdt(mat, net, trees=100, min_leaf=5, n_jobs=4, seed=42, verbose=False):
    ...
    for i in tqdm(range(mat.shape[0]), disable=not verbose):
        if isinstance(mat, csr_matrix):
            ...
            sample = mat[i]
        acts[i] = fit_rf(
            sr, net, sample, trees=trees, min_leaf=min_leaf, n_jobs=n_jobs, seed=seed
        )
