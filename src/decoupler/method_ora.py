...
from decoupler.pre import break_ties, extract, filt_min_n, rename_net, return_data
from decoupler.utils import p_adjust_fdr


@nb.njit(nb.f8(nb.i8, nb.i8, nb.i8, nb.i8), cache=True)
def mlnTest2r(a, ab, ac, abcd):
    if 0 > a or a > ab or a > ac or ab + ac > abcd + a:
        msg = "invalid contingency table"
...
def get_pvals(sample, net, starts, offsets, n_background, n_table):
    ...
    for i in nb.prange(n_fsets):
        ...
        odds_r[i] = ((a + 0.5) * (n_background - size + 0.5)) / (
            (size + 0.5) * (nfeatures - a + 0.5)
        )
...
def ora(mat, net, n_up_msk, n_bt_msk, n_background=20000, verbose=False):
    ...
    for i in tqdm(range(n_samples), disable=not verbose):
        ...
        # Estimate pvals
        _, _, _, pvls[i], _ = get_pvals(
            sample, net, starts, offsets, n_background, n_features
        )
...
def extract_c(df):
    if isinstance(df, pd.DataFrame):
        ...
        msg = "df must be a dataframe with significant features as indexes, or a list/array of features."
        raise ValueError(msg)
    return c


def get_ora_df(
    df, net, source="source", target="target", n_background=20000, verbose=False
):
    """
    ...
    if n_background is None:
        ...
        c = c[msk]
        if c.size == 0:
            msg = """No features in df match with the target features of net. Check that df contains enough
            features or that you have specified the correct 'target' column in net."""
            raise ValueError(msg)
    elif not isinstance(n_background, int):
        msg = "n_background must be a positive integer or None."
        raise ValueError(msg)
    ...
    # Estimate pvals
    sizes, overlap_r, odds_r, pvls, overlap = get_pvals(
        idxs, net, starts, offsets, n_background, n_features
    )
    ...
    res.insert(
        6, "Combined score", -np.log(res["p-value"].values) * res["Odds ratio"].values
    )
...
def run_ora(
    ...
    if not 0 <= n_up:
        msg = "n_up needs to be a value higher than 0."
        raise ValueError(msg)
    if not 0 <= n_bottom:
        msg = "n_bottom needs to be a value higher than 0."
        raise ValueError(msg)
    if not 0 <= n_background:
        msg = "n_background needs to be a value higher than 0."
        raise ValueError(msg)
    if not (len(c) - n_up) >= n_bottom:
        msg = "n_up and n_bottom overlap, please decrase the value of any of them."
        raise ValueError(msg)
    n_up_msk = len(c) - n_up
    n_bt_msk = n_bottom + 1
