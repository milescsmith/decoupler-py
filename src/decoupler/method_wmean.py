...
from decoupler.method_gsea import std
...
@nb.njit(
    nb.types.UniTuple(nb.f4[:, :], 3)(
        nb.f4[:, :], nb.f4[:, :], nb.f4[:, :], nb.i8[:], nb.f4[:], nb.i8, nb.i8
    ),
    cache=True,
)
...
def wmean(mat, net, times, batch_size, seed, verbose):
    ...
    if isinstance(mat, csr_matrix):
        ...
        for i in tqdm(range(n_batches), disable=not verbose):
            ...
            if times > 1:
                norm[srt:end], corr[srt:end], pvals[srt:end] = run_perm(
                    estimate[srt:end], tmp, net, idxs, div, times, seed
                )
    else:
