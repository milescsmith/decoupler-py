...
@nb.njit(
    nb.types.Tuple((nb.f4, nb.i8, nb.f4[:]))(nb.f4[:], nb.i8[:], nb.b1[:], nb.f4),
    cache=True,
    error_model="numpy",
)
def compute_es_per_rank(row, rnks, set_msk, dec):
    ...
    if sum_set == 0.0:
        return 0.0, 0, np.zeros(rnks.size, dtype=nb.f4)
...
@nb.njit(
    nb.types.UniTuple(nb.f4, 2)(
        nb.f4[:], nb.i8[:], nb.b1[:], nb.f4, nb.f4, nb.i8, nb.i8
    ),
    cache=True,
    error_model="numpy",
)
...
@nb.njit(
    nb.types.Tuple(
        (nb.f4[:], nb.f4[:], nb.f4[:], nb.i8[:], nb.f4[:], nb.f4[:], nb.b1[:, :])
    )(nb.f4[:], nb.i8[:], nb.i8[:], nb.i8[:], nb.i8, nb.i8, nb.b1),
    parallel=True,
    cache=True,
    error_model="numpy",
)
def nb_gsea(row, net, starts, offsets, times, seed, ratios):
    ...
    for j in nb.prange(n_fsets):
        ...
        # Compute nes
        if times > 0:
            nes[j], pval[j] = compute_nes_per_rank(
                row, rnks, set_msk, dec, estimate, times, seed
            )
...
def gsea(mat, net, times=1000, seed=42, verbose=False):
    ...
    for i in tqdm(range(n_samples), disable=not verbose):
        ...
        es[i], nes[i], pvals[i], _, _, _, _ = nb_gsea(
            row, net, starts, offsets, times, seed, False
        )
...
def get_gsea_df(
    ...
    cols = df.columns
    if stat not in cols:
        msg = f'Column name "{stat}" for stat not found in df. Please specify a valid column.'
    ...
    # Compute GSEA per row
    es, nes, pvals, sizes, hits_r, rnks_r, le_msk = nb_gsea(
        m, net, starts, offsets, times, seed, True
    )
