...
from decoupler.method_gsea import compute_es_per_rank
from decoupler.pre import extract, filt_min_n, rename_net
from decoupler.utils_anndata import (
    get_cpm,
    get_cpm_cutoff,
    get_filterbyexpr_inputs,
    get_min_sample_size,
)
...
def check_if_matplotlib(return_mpl=False):
    if not return_mpl:
        try:
            ...
            msg = "matplotlib is not installed. Please install it with: pip install matplotlib"
...
def check_if_seaborn():
    try:
        ...
        msg = "seaborn is not installed. Please install it with: pip install seaborn"
        raise ImportError(msg)
    return sns
...
def check_if_adjustText():
    try:
        ...
        msg = "adjustText is not installed. Please install it with: pip install adjustText"
        raise ImportError(msg)
    return at
...
def check_if_igraph():
    try:
        ...
        msg = "igraph is not installed. Please install it with: pip install igraph"
        raise ImportError(msg)
    from packaging.version import Version

    if Version(ig.__version__) < Version("0.10.0"):
        msg = "igraph version needs to be at least 0.10.0. Please install it with: pip install igraph==0.10.0"
        raise ImportError(msg)
    return ig
    ...
    if save is not None:
        if ax is not None:
            if fig is not None:
                ...
                msg = "fig is None, cannot save figure."
                raise ValueError(msg)
        else:
            msg = "ax is None, cannot save figure."
            raise ValueError(msg)
...
def plot_volcano(
    ...
    if net is not None:
        ...
        df.loc[(df["logFCs"] >= lFCs_thr) & (df["pvals"] >= sign_thr), "weight"] = (
            color_pos
        )
        df.loc[(df["logFCs"] <= -lFCs_thr) & (df["pvals"] >= sign_thr), "weight"] = (
            color_neg
    ...
    signs = df[
        (np.abs(df["logFCs"]) >= lFCs_thr) & (df["pvals"] >= sign_thr)
    ...
    if len(texts) > 0:
        at.adjust_text(texts, arrowprops={"arrowstyle": "-", "color": "black"}, ax=ax)
...
def plot_targets(
    ...
    pos = ((data[weight] >= 0) & (data[stat] >= 0)) | (
        (data[weight] < 0) & (data[stat] < 0)
    )
    data["color"] = "#1F77B4"
...
def plot_barplot(
    ...
    if np.any(~np.isfinite(acts)):
        msg = "Input acts contains non finite values."
        raise ValueError(msg)
    ...
    df = (
        ...
        .iloc[:, np.argsort(abs(df.values))[0]]
        .T
...
def plot_metrics_scatter(
    ...
    for cat, msk in zip(cats, msks):
        ...
        sub = (
            ...
            .reset_index()
            .pivot(index="method", columns="metric", values="score")
            .reset_index()
        )
        ...
        if show_text:
            text = [
                ax.text(x_vals[i], y_vals[i], s_vals[i], zorder=2)
    ...
    if show_text:
        at.adjust_text(texts, arrowprops={"arrowstyle": "-", "color": "gray"}, ax=ax)
            ...
            if is_empty:
                for m in metrics:
                    if col == "method" or groupby == "method":
                        tmp = pd.DataFrame(
                            [[cat, grpby, m, np.nan, np.nan]],
                            columns=[col, groupby, "metric", "score", "ci"],
                        )
                    else:
                        tmp = pd.DataFrame(
                            [[cat, grpby, "", m, np.nan, np.nan]],
...
def plot_metrics_boxplot(
    ...
    **kwargs,
    ...
    if metric not in ["mcauroc", "mcauprc", "rank", "nrank", "recall"]:
        msg = 'Argument metric must be either "mcauroc", "mcauprc", "rank" or "nrank".'
        raise ValueError(msg)
    ...
    if type(groupby) is str and groupby != "method":
        # Compute order
        order = (
            df.groupby(["method", groupby], observed=True)
            .mean(numeric_only=True)
            .reset_index()
        ...
        sns.boxplot(
            ...
            ax=ax,
            order=order,
            hue_order=np.sort(df[groupby].unique()),
            **kwargs,
        )
        ...
        # Compute order
        order = (
            df.groupby(["method"]).mean(numeric_only=True).sort_values("score").index
        )

        sns.boxplot(x="method", y="score", data=df, ax=ax, order=order, **kwargs)

    else:
        msg = 'Argument groupby must be a string and not be "method".'
        raise ValueError(msg)
...
def plot_psbulk_samples(
    ...
    **kwargs,
    ...
    if type(groupby) is list and ax is not None:
        msg = """If a grupby is a list of columns ax must be None."""
        raise ValueError(msg)
    elif type(groupby) is list:
        fig, axes = plt.subplots(
            1, len(groupby), figsize=figsize, dpi=dpi, tight_layout=True
        )
        axes = axes.ravel()
        for ax, grp in zip(axes, groupby):
            ...
            ax.legend(
                loc="center left", bbox_to_anchor=(1, 0.5), frameon=False, title=grp
            )
        ...
        ax.legend(
            loc="center left", bbox_to_anchor=(1, 0.5), frameon=False, title=groupby
        )
        ax.set_xlabel("Log10 number of cells")
        ax.set_ylabel("Log10 total sum of counts")
...
def plot_filter_by_expr(
    ...
    **kwargs,
    ...
    total_counts[total_counts < 1.0] = np.nan  # Handle 0s
    ...
    if ax is None:
        fig, ax = plt.subplots(1, 1, figsize=figsize, dpi=dpi)
    sns.histplot(
        x=np.log10(total_counts),
        y=sample_size,
        cmap=cmap,
        cbar=True,
        cbar_kws={"shrink": 0.75, "label": "Number of features"},
        discrete=(False, True),
        ax=ax,
        **kwargs,
    )
...
def plot_running_score(
    ...
    # Compute es
    mx_value, j, es = compute_es_per_rank(
        m.astype(np.float32), rnks.astype(np.int64), set_msk.astype(bool), dec
    )
...
def _check_assoc_plot_intputs(
    data, associations, cols, uns_key, obsm_key, use_X, layer, stat_col
):
    ...
    assert hasattr(data, "uns") and (
        hasattr(data, "obsm") or hasattr(data, "X") or hasattr(data, "layers")
    ), msg
    ...
    assert (
        stat_col in stats.columns
    ), "stat_col must be one of the columns in the anova results"
    ...
    assert (
        obsm_key is not None or use_X is True or layer is not None
    ), "one of obsm_key, use_X, or layer must be specified"
    if (
        (obsm_key is not None and use_X)
        or (obsm_key is not None and layer is not None)
        or (use_X and layer is not None)
    ):
        msg = "only one of obsm_key, use_X, or layer should be specified"
        raise ValueError(msg)
    ...
    if obsm_key is not None:
        ...
        if isinstance(data.obsm[obsm_key], pd.DataFrame):
            ...
            acts = pd.DataFrame(
                data.obsm[obsm_key], index=data.obs.index, columns=columns
            )
...
def plot_associations(
    ...
    try:
        ...
        msg = (
            'marsilea is not installed. Please install it using "pip install marsilea"'
        )
        raise ImportError(msg)

    # Extract inputs
    stats, acts = _check_assoc_plot_intputs(
        data, associations, cols, uns_key, obsm_key, use_X, layer, stat_col
    )

    # Go from long to wide format using only the selected summary statistic
    col_order = stats["factor"].unique()
    stats = stats.pivot(index="factor", columns="variable", values=stat_col).T.loc[
        :, col_order
    ]
    ...
    layer = mp.MarkerMesh(sign_msk, marker="*", label=f"{stat_col} < {sign_thr!s}")
    ...
    c = h1 / 0.2 / h2
    c.add_legends(side="right", stack_by="row", stack_size=2)
    c.render()
...
def get_source_idxs(n_sources, act, by_abs):
    if isinstance(n_sources, str):
        ...
        msg = f"n_sources needs to be a list of source names or an \
        integer number, {type(n_sources)} was passed."
        raise ValueError(msg)
    return s_idx
...
def get_target_idxs(n_targets, obs, net, by_abs):
    if isinstance(n_targets, str):
        ...
        net["prod"] = [
            obs.iloc[0][t] * w if t in obs.columns else 0
            for t, w in zip(net["target"], net["weight"])
        ]
        if by_abs:
        ...
        msg = f"n_targets needs to be a list of target names or an \
        integer number, {type(n_targets)} was passed."
        raise ValueError(msg)
    return t_idx
...
def get_obs_act_net(act, obs, net, n_sources, n_targets, by_abs):
    ...
    assert np.all(obs.index == act.index) and (
        obs.index.size == 1
    ), "obs and act need to have the same row index."
