...
def _check_if_pypath() -> None:
    ...
    if getattr(pypath, "__version__", None) and ver(pypath.__version__) < ver(
        PYPATH_MIN_VERSION
    ):
        msg = (
...
def _the_organism(organism: str | int) -> str:
    return "mouse" if _is_mouse(organism) else "rat" if _is_rat(organism) else "human"


def _static_fallback(
    query: str, resource: str, organism: int | str, **kwargs
) -> pd.DataFrame:
    """
    ...
    return op.static.static_table(
        query=query, resource=resource, organism=organism, **kwargs
    )
...
def get_progeny(
    ...
    top: int = 100,
    genesymbol_resource: (
        Literal["uniprot", "ensembl"] | dict[str, set[str]] | bool | None
    ) = None,
    **kwargs,
) -> pd.DataFrame:
    """
...
def get_resource(
    ...
    **kwargs,
    ...
    if annot_resources:
        if name not in annot_resources:
            msg = (
    ...
    try:
        df = op.requests.Annotations.get(
            resources=name, entity_type="protein", **kwargs
        )
    ...
    **kwargs,
    ...
    try:
        ct = op.interactions.CollecTRI.get(
            genesymbols=True, organism=_organism, loops=True, **kwargs
        )
...
def translate_net(
    ...
    **kwargs: dict[str, str],
    ...
    from pypath.utils import orthology, taxonomy
...
def add_genesymbols(
    ...
    target_column: str,
    organism: str | int = "human",
    resource: (Literal["uniprot", "ensembl"] | dict[str, set[str]]) = "uniprot",
) -> pd.DataFrame:
    """
    ...
    if not isinstance(resource, dict):
        if resource == "uniprot":
            ...
            msg = 'Resource must be either "uniprot" or "ensembl".'
            raise ValueError(msg)
    ...
    mapping_table = pd.DataFrame(
        ((u, g) for u, gs in resource.items() for g in gs),
    ...
    net = net[
        [*list(net.columns[0:col_idx]), target_column, *list(net.columns[col_idx:-1])]
    ]
...
def get_ksn_omnipath(
    ...
    ksn["target"] = [
        f"{sub}_{res}{off}"
        for sub, res, off in zip(
            ksn["substrate_genesymbol"], ksn["residue_type"], ksn["residue_offset"]
        )
    ]

    # Assigns mode of regulation
    ksn["weight"] = [
        +1 if mod == "phosphorylation" else -1 for mod in ksn["modification"]
    ]

    # Remove duplicates
    ksn = ksn.rename(columns={"enzyme_genesymbol": "source"})[
