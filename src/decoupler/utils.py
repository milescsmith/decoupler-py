...
from decoupler.pre import extract, filt_min_n, get_net_mat, rename_net
...
def melt(df):
    ...
    # If input is an individual dataframe
        ...
        msg = f"Input type {type(df)} not supported."
...
def check_corr(
    ...
    corr = pd.DataFrame(
        np.triu(corr, k=1), index=sources, columns=sources
    ).reset_index()
    corr = corr.melt(id_vars="index").rename(
        {"index": "source1", "variable": "source2", "value": "corr"}, axis=1
    )
    corr = corr[corr["corr"] != 0]

    # Sort by abs value
    corr = corr.iloc[np.argsort(np.abs(corr["corr"].values))[::-1]].reset_index(
        drop=True
    )
...
def get_toy_data(n_samples=24, seed=42):
    ...
    features = [f"G{i+1:02d}" for i in range(n_features)]
...
def summarize_acts(acts, groupby, obs=None, mode="mean", min_std=1.0):
    ...
    if type(acts) is AnnData:
        if obs is not None:
            msg = "If acts is AnnData, obs needs to be None."
            raise ValueError(msg)
        ...
        for i in range(n_groups):
            ...
            for j in range(n_features):
                ...
                if mode == "mean":
                    ...
                    msg = "mode can only be either mean or median."
                    raise ValueError(msg)
...
def dense_run(
    ...
    # Name
    acts.name = f"{fname}_estimate"
    pvals.name = f"{fname}_pvals"
...
def shuffle_net(net, target=None, weight=None, seed=42, same_seed=True):
    ...
    if target is None and weight is None:
        msg = "If target and weight are None, nothing is shuffled."
        raise ValueError(msg)

    if target is not None:
        if target not in rnet.columns:
            msg = f'Colum target="{target}" not in rnet. Specify a valid column name.'
            raise ValueError(msg)
        else:
            rng = default_rng(seed=seed)
    ...
    if weight is not None:
        if weight not in rnet.columns:
            msg = f'Colum weight="{weight}" not in rnet. Specify a valid column name.'
            raise ValueError(msg)
        else:
            if not same_seed:
                seed = seed + 1
