...
from decoupler.utils import melt, p_adjust_fdr
...
def get_acts(adata, obsm_key, dtype=np.float32):
    ...
    return AnnData(
        np.array(adata.obsm[obsm_key]).astype(dtype),
        obs=obs,
        var=var,
        uns=uns,
        obsm=obsm,
...
def extract_psbulk_inputs(
    adata: AnnData, obs: pd.DataFrame, layer: str, use_raw: bool
) -> tuple[csr_matrix, pd.DataFrame, pd.DataFrame]:
    # Extract count matrix X
    if layer is not None:
        x_arr = adata.layers[layer]
    elif isinstance(adata, AnnData):
        if use_raw:
            if adata.raw is None:
                msg = "Received `use_raw=True`, but `mat.raw` is empty."
    ...
    if isinstance(adata, AnnData):
        ...
        if obs is None:
            msg = "If adata is a pd.DataFrame, obs cannot be None."
            raise ValueError(msg)
        ...
        try:
            ...
            msg = "Indices in obs do not match with mat's."
    ...
    if issparse(x_arr) and not isinstance(x_arr, csr_matrix):
        x_arr = csr_matrix(x_arr)

    return x_arr, obs, var
...
def check_X(X, mode="sum", skip_checks=False):
    ...
    if not is_finite:
        msg = "Data contains non finite values (nan or inf), please set them to 0 or remove them."
        raise ValueError(msg)
    skip_checks = isinstance(mode, dict) or callable(mode) or skip_checks
    if not skip_checks:
        if isinstance(X, csr_matrix):
        ...
        if not is_positive:
            msg = """Data contains negative values. Check the parameters use_raw and layers to
        ...
        if mode == "sum":
            ...
            if not is_integer:
                msg = """Data contains float (decimal) values. Check the parameters use_raw and layers to
...
def psbulk_profile(profile, mode="sum"):
    if mode == "sum":
        ...
        msg = f"""mode={mode} can be 'sum', 'mean', 'median' or a callable function."""
        raise ValueError(msg)
    return profile
...
def get_pseudobulk(
    ...
    # Extract inputs
    x_arr, obs, var = extract_psbulk_inputs(adata, obs, layer, use_raw)

    # Test if X is correct
    check_X(x_arr, mode=mode, skip_checks=skip_checks)
    ...
    if isinstance(mode, dict):
        ...
        for l_name in mode:
            ...
            if not callable(func):
                msg = (
                    f"mode requieres a dictionary of layer names and callable functions. "
                    f"The layer {l_name} does not contain one."
                )
                raise ValueError(msg)
            else:
                # Compute psbulk
        ...
        layers = dict(zip(mode.keys(), psbulks))
        layers["psbulk_props"] = props
    elif isinstance(mode, str) or callable(mode):
        # Compute psbulk
        psbulk, ncells, counts, props = compute_psbulk(
            n_rows,
            n_cols,
    ...
    if isinstance(mode, dict):
        swap_layer(
            psbulk, layer_key=next(iter(mode.keys())), X_layer_key=None, inplace=True
        )
...
def get_contrast(
    adata, group_col, condition_col, condition, reference=None, method="t-test"
):
    """
    ...
    if group_col is not None:
        ...
        group_col = "tmpcol"
        grp = f"{condition}.vs.{reference}"
        adata.obs[group_col] = grp
        groups = [grp]

    # Condition and reference must be different
    if condition == reference:
        msg = "condition and reference cannot be identical."
        raise ValueError(msg)
    ...
    for grp in groups:
        ...
        if (
            sub_adata.obs[condition_col].dtype == "object"
            or sub_adata.obs[condition_col].dtype == "category"
        ):
...
def get_top_targets(
    ...
    if net is not None:
        ...
        if name is None:
            msg = "If net is given, name cannot be None."
            raise ValueError(msg)
    ...
    # Order columns
    if fdr_corr:
        df = df[["contrast", "name", "logFCs", "pvals", "adj_pvals"]].sort_values(
            "pvals"
        )
    else:
...
def format_contrast_results(logFCs, pvals):
    ...
    df = melt([logFCs, pvals]).rename(
        {"sample": "contrast", "source": "name", "score": "logFCs"}, axis=1
    )
    df = (
        df[["contrast", "name", "logFCs", "pvals"]]
        .sort_values("contrast")
        .reset_index(drop=True)
    )
    df["adj_pvals"] = (
        df.groupby("contrast", observed=True)
...
def get_filterbyexpr_inputs(adata, obs):
    ...
    if isinstance(adata, AnnData):
        ...
        msg = """adata must be either an AnnData object or a df."""
        raise ValueError(msg)
...
def get_min_sample_size(group, obs, large_n, min_prop):
    ...
    if group is None:
        ...
        msg = "group needs to be a column name, or a list/series/array of values."
        raise ValueError(msg)
...
def get_cpm(y, lib_size):
    if isinstance(lib_size, float) or isinstance(lib_size, int):
...
def filter_by_prop(adata, min_prop=0.2, min_smpls=2):
    ...
    if isinstance(adata, AnnData):
        ...
        if "psbulk_props" in list(layer_keys):
            ...
            genes = var_names[msk]
            return genes
    msg = """adata must be an AnnData object that contains the layer 'psbulk_props'. Please check the function
                     decoupler.get_pseudobulk."""
    raise ValueError(msg)


def rank_sources_groups(
    adata, groupby, reference="rest", method="t-test_overestim_var"
):
    """
    ...
    for group in groups:
        ...
        if np.sum(ref_msk) <= 0:
            msg = f"No reference samples found for {reference}"
            raise ValueError(msg)
        ...
        for i in np.arange(len(features)):
            ...
            if not (np.isfinite(v_group).all() and np.isfinite(v_rest).all()):
                msg = "adata contains not finite values, please remove them."
                raise ValueError(msg)
            if method == "wilcoxon":
                ...
                msg = "Method must be one of {'wilcoxon', 't-test', 't-test_overestim_var'}."
                raise ValueError(msg)
...
def _check_anova_inputs(data, obs_keys=None, obsm_key=None, use_X=False, layer=None):
    ...
    if isinstance(data, list):
        required_list_size = Literal[2]
        if len(data) != required_list_size:
            msg = "data must be a list of size 2"
            raise ValueError(msg)
        # both elements in data must be pd.Dataframe instances
        if not isinstance(data[0], pd.DataFrame):
            msg = "data[0] must be a pd.DataFrame instance"
            raise ValueError(msg)
        if not isinstance(data[1], pd.DataFrame):
            msg = "data[1] must be a pd.DataFrame instance"
            raise ValueError(msg)
        # both data[0] and data[1] must have the same index
        if not data[0].index.equals(data[1].index):
            msg = "data[0] and data[1] must have the same index"
            raise ValueError(msg)
        ...
        if not isinstance(data.obs, pd.DataFrame):
            "data.obs must be a pd.DataFrame instance"
        obs = data.obs
        if obsm_key is None and not use_X and layer is None:
            msg = "When providing an AnnData or MuData object, either obsm_key, use_X or layer must be specified"
            raise ValueError(msg)
        elif (
            (obsm_key is not None and use_X)
            or (obsm_key is not None and layer is not None)
            or (use_X and layer is not None)
        ):
            msg = (
                "When providing an AnnData or MuData object, only one of obsm_key, "
                "use_X or layer must be specified"
            )
            raise ValueError(msg)
        elif obsm_key is not None:
            if not hasattr(data, "obsm"):
                msg = "data must have an .obsm attribute"
                raise ValueError(msg)
            if obsm_key not in data.obsm.keys():
                msg = "obsm_key must be a key in data.obsm"
                raise ValueError(msg)
            column_name = (
                obsm_key.replace("X_", "")
                .replace("pca", "PC")
            ...
            scores = pd.DataFrame(
                ...
                columns=[
                    f"{column_name}{1 + x}" for x in range(data.obsm[obsm_key].shape[1])
                ],
            )
        elif use_X:
            if not hasattr(data, "X"):
                msg = "data must have a .X attribute"
                raise ValueError(msg)
            scores = pd.DataFrame(data.X, index=data.obs.index, columns=data.var.index)
        elif layer is not None:
            if not hasattr(data, "layers"):
                msg = "data must have a .layers attribute"
                raise ValueError(msg)
            if layer not in data.layers.keys():
                msg = "layer must be a key in data.layers"
                raise ValueError(msg)
            scores = pd.DataFrame(
                data.layers[layer], index=data.obs.index, columns=data.var.index
            )

        if not scores.index.equals(obs.index):
            msg = "scores and obs must have the same index"
            raise ValueError(msg)
        dependent_variables = scores.columns
        explanatory_variables = obs.columns
    else:
        msg = "data must be a list of size 2 or an AnnData or MuData object with .obs dataframe"
        raise TypeError(msg)

    # select only the columns in obs_keys
    if obs_keys is None:
        obs_keys = explanatory_variables
    if not all(var in explanatory_variables for var in obs_keys):
        msg = "obs_keys must be a subset of the obs columns"
        raise ValueError(msg)
    ...
    if not all("." not in var for var in dependent_variables):
        msg = "column names of dependent variables cannot contain ."
        raise ValueError(msg)
    if not all("." not in var for var in explanatory_variables):
        msg = "column names of explanatory variables (obs) cannot contain ."
        raise ValueError(msg)
    # check there is no overlap of column names between dependent and explanatory variables
    if (
        len([value for value in dependent_variables if value in explanatory_variables])
        != 0
    ):
        msg = "dependent and explanatory variables cannot have the same column names"
        raise ValueError(msg)
...
def get_metadata_associations(
    ...
    scores, dependent_variables, explanatory_variables = _check_anova_inputs(
        data, obs_keys=obs_keys, obsm_key=obsm_key, use_X=use_X, layer=layer
    )
    ...
    for dependent in tqdm(dependent_variables, disable=not verbose):
        ...
        for explainer in explanatory_variables:
            ...
            if (
                scores[explainer].dtype == "object"
                or scores[explainer].dtype == "category"
            ):
                # if it is a string, the explanatory variable is a categorical variable
                var_name = f"C({explainer})"
            # if it is a float or integer, the explanatory variable is a continuous variable
                ...
                var_name = f"{explainer}"
            # else raise an error
            else:
                msg = f"Column {explainer} has an unknown data type. Make sure it is either \
                categorical/object or float/int"
                raise ValueError(msg)

            # create the formula
            formula = f"{dependent} ~ {var_name}"
            ...
            try:
                ...
                eta_sq = (
                    aov_table.loc[var_name, "sum_sq"] / aov_table.sum(axis=0)["sum_sq"]
                )
        ...
        stats = pd.DataFrame(stats, columns=["variable", "pval", "eta_sq"]).set_index(
            "variable"
        )
    ...
    return format_assoc_results(
        data, stats_df, inplace, obsm_key, uns_key, use_X, layer
    )
