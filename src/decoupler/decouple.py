...
from .utils import dense_run, show_methods
...
def get_wrappers(methods):
    ...
    for method in methods:
        try:
            ...
            msg = f"""Method {method} not available, please run show_methods() to see the list of available
            methods."""
...
def run_methods(
    mat, net, source, target, weight, methods, args, min_n, verbose, use_raw, dense
):
...
def parse_methods(methods, cns_metds):
    ...
    if methods is None:
        ...
        if methods.lower() == "all":
            methods = [
                method.split("run_")[1]
                for method in show_methods()["Function"]
...
def decouple(
    ...
    if consensus:
        if cns_metds is not None:
            if not isinstance(cns_metds, list):
                cns_metds = [cns_metds]
