...
def density(mat, kcdf):
    ...
    if kcdf is None:
        ...
        if kcdf == "gaussian":
            ...
            msg = "kcdf needs to be either 'gaussian', 'poisson', or None"
...
@nb.njit(
    nb.types.Tuple((nb.i8[:, :], nb.i8[:, :]))(nb.f8[:, :]), parallel=True, cache=True
)
...
@nb.njit(
    nb.types.UniTuple(nb.f8, 2)(nb.i8[:], nb.i8, nb.i8[:], nb.i8[:], nb.i8), cache=True
)
def rnd_walk(gsetidx, k, generanking, rankstat, n):
    ...
    for i in range(n):
        wlkstat = (stepcdfingeneset[i] / stepcdfingeneset[-1]) - (
            stepcdfoutgeneset[i] / stepcdfoutgeneset[-1]
        )
        walkstat[i] = wlkstat
        walkstatpos = max(wlkstat, walkstatpos)
        walkstatneg = min(wlkstat, walkstatneg)
...
@nb.njit(
    nb.f8[:](nb.i8[:, :], nb.i8[:, :], nb.i8[:], nb.b1, nb.b1),
    parallel=True,
    cache=True,
)
