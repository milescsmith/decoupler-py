...
from decoupler.utils import shuffle_net
from decoupler.utils_benchmark import get_toy_benchmark_data

# rewrite this to use fixture
...
def test_get_performances():
    ...
    get_performances(
        res, obs, groupby=["col_B"], by="experiment", metrics=["auroc"], min_exp=1
    )
...
def test_benchmark():
    ...
    rnet = shuffle_net(net, target="target", weight="weight").drop_duplicates(
        ["source", "target"]
    )
    nets = {"net": net, "rnet": rnet}
    decouple_kws = {"net": {"min_n": 0}, "rnet": {"min_n": 0}}
    df = benchmark(
        mat,
        obs,
        nets,
