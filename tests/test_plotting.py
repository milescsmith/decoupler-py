...
def test_plot_metrics_scatter():
    ...
    plot_metrics_scatter(
        df, x="mcauroc", y="mcauprc", groupby=None, title="title", return_fig=False
    )
    plot_metrics_scatter(
        df, x="mcauroc", y="mcauprc", groupby="net", title="title", return_fig=True
    )
...
def test_plot_metrics_scatter_cols():
    ...
    plot_metrics_scatter_cols(
        df, col="net", x="mcauroc", y="mcauprc", groupby="method", return_fig=True
    )
    ...
    plot_metrics_scatter_cols(
        df, col="source", x="mcauroc", y="mcauprc", groupby="net", return_fig=True
    )
    plot_metrics_scatter_cols(
        df, col="method", x="mcauroc", y="mcauprc", groupby="net", return_fig=True
    )
...
def test_plot_psbulk_samples():
    obs = pd.DataFrame(
        ...
        columns=["sample_id", "cell_type", "psbulk_n_cells", "psbulk_counts"],
        index=["1", "2", "3", "4"],
    )
...
def test_get_obs_act_net():
    ...
    fact, fobs, fnet = get_obs_act_net(
        act, obs, net, n_sources=2, n_targets=1, by_abs=False
    )
    ...
    fact, fobs, fnet = get_obs_act_net(
        act, obs, net, n_sources=2, n_targets=1, by_abs=True
    )
...
def test_plot_network():
    ...
    plot_network(
        net, obs, act, figsize=(3, 3), node_size=0.25, s_cmap="red", t_cmap="blue"
    )
