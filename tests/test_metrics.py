...
def test_mc_perm():
    ...
    assert np.all(y_score[:, 0] == 7.0)
    assert np.all(y_score[:, 1] == 4.0)
    assert np.all(y_score[:, 2] == 3.0)
...
def test_metric_rank():
    ...
    assert a.size == 4
    assert np.all(a == np.array([1.0, 2.0, 3.0, 3.0]))
...
def test_metric_nrank():
    ...
    assert a.size == 4
    assert np.all(a == np.array([0.0, 0.5, 1.0, 1.0]))
