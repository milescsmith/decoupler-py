...
from decoupler.method_ora import run_ora
...
def test_melt():
    ...
    pvals = pd.DataFrame(
        [[0.005, 0.5, 0.7], [0.006, 0.6, 0.9], [0.004, 0.3, 0.7]],
        columns=["T1", "T2", "T3"],
        index=["S01", "S02", "S03"],
    )
...
def test_check_corr():
    mat = pd.DataFrame(
        [[1, 2, 3, 4, 5, 6]], columns=["G01", "G02", "G03", "G06", "G07", "G08"]
    )
    net = pd.DataFrame(
        [
            ["T1", "G01", 1],
...
def test_summarize_acts():
    ...
    obs = pd.DataFrame(
        [["C01", "C01", "C02"]], columns=estimate.index, index=["celltype"]
    ).T
    adata = AnnData(estimate.astype(np.float32), obs=obs)
    acts = summarize_acts(
        estimate, obs=obs, groupby="celltype", mode="median", min_std=0
    )
...
def test_shuffle_net():
    ...
    net_dict = dict(zip(net.target, net.weight))
    rnet_dict = dict(zip(rnet.target, rnet.weight))
...
def test_read_gmt():
    ...
    counts = df.groupby("source", group_keys=False)["target"].apply(
        lambda x: np.array(x)
    )
