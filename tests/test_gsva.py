...
from decoupler.method_gsva import density, gsva, init_cdfs, norm_cdf, ppois, run_gsva
...
def test_density():
    m = np.array(
        [[4, 5, 1, 0], [6, 4, 0, 2], [8, 8, 3, 1], [1, 1, 9, 8], [0, 2, 7, 5]],
        dtype=np.float32,
    )
    d_gauss = density(m, kcdf="gaussian")
    assert np.all(
        np.isclose(
            d_gauss[0, :], np.array([0.006604542, 0.77949374, -0.97600555, -1.9585940])
        )
    )
    assert np.all(
        np.isclose(
            d_gauss[:, 0],
            np.array(
                [0.006604542, 0.847297859, 2.178640849, -0.960265677, -1.962387678]
            ),
        )
    )
    ...
    assert np.all(
        np.isclose(
            d_poiss[0, :], np.array([0.2504135, 0.6946200, -0.74551495, -1.49476762])
        )
    )
    assert np.all(
        np.isclose(
            d_poiss[:, 0],
            np.array([0.2504135, 0.9572208, 1.7733889, -0.8076761, -1.5963288]),
        )
    )
...
def test_ppois():
    assert ppois(1000, 3) == 1.0
    assert np.isclose(ppois(100, 75), 0.9975681)
    assert np.isclose(ppois(300000, 300000), 0.5004856)
    assert ppois(1, 1000) == 0.0


def test_norm_cdf():
    assert np.isclose(
        norm_cdf(np.array([1, 2, 3], dtype=float), mu=0.0, sigma=1.0),
        np.array([0.8413447, 0.9772499, 0.9986501]),
    ).all()
    assert np.isclose(
        norm_cdf(np.array([0, 9, 1], dtype=float), mu=0.0, sigma=1.0),
...
def test_gsva():
    ...
    net = pd.Series(
        [np.array([0, 1, 2], dtype=np.int64), np.array([1, 2, 3], dtype=np.int64)],
        index=["T1", "T2"],
    )
...
def test_run_gsva():
    ...
    res_lst = run_gsva([m, r, c], net, min_n=0, use_raw=False, verbose=True, kcdf=None)[
        0
    ].values
    assert np.isclose(res_lst[0, :], np.array([1, 0.3333333])).all()
    assert np.isclose(
        res_lst[:, 0], np.array([1, 0.3333333, 1, 0.3333333, 0.3333333])
    ).all()
