...
from decoupler.utils_anndata import (
    ...
    check_X,
    compute_psbulk,
    extract_psbulk_inputs,
    filter_by_expr,
    filter_by_prop,
    format_contrast_results,
    format_psbulk_inputs,
    get_acts,
    get_contrast,
    get_cpm,
    get_cpm_cutoff,
...
def acts_df():
    return pd.DataFrame(
        data=np.array([[1, 0, 2], [1, 0, 3], [0, 0, 0]]),
        index=np.array(["S1", "S2", "S3"]),
...
@pytest.fixture
def acts_adata(acts_df, acts_estimate):
    adata = AnnData(acts_df.astype(np.float32))
    adata.obsm["estimate"] = acts_estimate
    return adata


def test_get_acts(acts_adata):
    acts = get_acts(acts_adata, "estimate")

    assert acts.shape[0] == acts_adata.shape[0]
    assert acts.shape[1] != acts_adata.shape[1]
    assert np.any(acts.X < 0)
    assert not np.any(acts_adata.X < 0)


@pytest.fixture
def swap_layer_df():
    return pd.DataFrame(
        data=np.array([[1, 0, 2], [1, 0, 3], [0, 0, 1]]),
        index=np.array(["S1", "S2", "S3"]),
        columns=np.array(["G1", "G2", "G3"]),
    )
...
@pytest.fixture
def swap_layer_adata(swap_layer_df):
    adata = AnnData(swap_layer_df.astype(np.float32))
    adata.layers["norm"] = adata.X / np.sum(adata.X, axis=1).reshape(-1, 1)
    return adata


def test_swap_layer(swap_layer_adata):
    sdata = swap_layer(
        swap_layer_adata, layer_key="norm", X_layer_key="x", inplace=False
    )
    assert not np.all(np.mod(sdata.X, 1) == 0)
    assert "x" in list(sdata.layers.keys())
    assert np.all(sdata.layers["x"] == swap_layer_adata.X)

    sdata = swap_layer(
        swap_layer_adata, layer_key="norm", X_layer_key=None, inplace=False
    )
    assert np.all(sdata.X == sdata.layers["norm"])
    assert "x" not in list(sdata.layers.keys())

    original_recipe = swap_layer_adata.copy()
    swap_layer(swap_layer_adata, layer_key="norm", X_layer_key="x", inplace=True)
    assert not np.all(np.mod(swap_layer_adata.X, 1) == 0)
    assert "x" in list(swap_layer_adata.layers.keys())

    swap_layer(original_recipe, layer_key="norm", X_layer_key=None, inplace=True)
    assert np.all(original_recipe.X == original_recipe.layers["norm"])
    assert "x" not in list(original_recipe.layers.keys())
...
def test_format_psbulk_inputs():
    ...
    new_obs, groups_col, smples, groups, n_rows = format_psbulk_inputs(
        "patient_id", groups_col="patient_id", obs=obs
    )
    ...
    assert n_rows == 2
    new_obs, groups_col, smples, groups, n_rows = format_psbulk_inputs(
        "patient_id", groups_col="celltype", obs=obs
    )
    assert new_obs.shape[1] != obs.shape[1]
    ...
    assert n_rows == 4
    new_obs, groups_col, smples, groups, n_rows = format_psbulk_inputs(
        "patient_id", groups_col=["sample_id", "celltype"], obs=obs
    )
    assert new_obs.shape[1] != obs.shape[1]
...
def test_compute_psbulk():
    ...
    assert np.sum(psbulk, axis=1)[2] == 0.0
    ...
    assert np.sum(psbulk, axis=1)[0] == 0.0
...
def test_get_pseudobulk():
    ...
    pdata = get_pseudobulk(
        adata, sample_col, groups_col, min_cells=0, min_counts=0, mode="sum"
    )
    assert pdata.shape[0] == 3
    assert np.all(pdata.X[0] == np.array([9.0, 3.0, 6.0]))
    pdata = get_pseudobulk(
        adata, sample_col, groups_col, min_cells=0, min_counts=0, mode="mean"
    )
    assert pdata.shape[0] == 3
    assert np.all(pdata.X[0] == np.array([3.0, 1.0, 2.0]))
    pdata = get_pseudobulk(
        adata, sample_col, groups_col, min_cells=0, min_counts=0, mode="median"
    )
    assert pdata.shape[0] == 3
    assert np.all(pdata.X[0] == np.array([2.0, 0.0, 2.0]))
    pdata = get_pseudobulk(
        adata,
        sample_col,
        groups_col,
...
def test_get_contrast():
    ...
    reference = "Ht"
    obs = pd.DataFrame(
        [["C1", "C1", "C1", "C1"], [condition, condition, reference, reference]],
        columns=r,
        index=[groups_col, condition_col],
    ).T
    adata = AnnData(df.astype(np.float32), obs=obs)
    ...
    obs = pd.DataFrame(
        [["C1", "C1", "C1", "C1"], [condition, condition, condition, reference]],
        columns=r,
        index=[groups_col, condition_col],
    ).T
    get_contrast(adata, groups_col, condition_col, condition, reference)
...
def test_get_top_targets():
    logFCs = pd.DataFrame(
        [[3, 0, -3], [1, 2, -5]], index=["C1", "C2"], columns=["G1", "G2", "G3"]
    )
    pvals = pd.DataFrame(
        [[0.3, 0.02, 0.01], [0.9, 0.1, 0.003]],
        index=["C1", "C2"],
        columns=["G1", "G2", "G3"],
    )
    contrast = "C1"
    name = "T1"
    net = pd.DataFrame(
        [["T1", "G1", 1], ["T1", "G2", 1], ["T2", "G3", 1], ["T2", "G4", 0.5]],
...
def test_get_filterbyexpr_inputs():
    samples = ["S1", "S2"]
    genes = ["G1", "G2", "G3"]
    df = pd.DataFrame([[2, 2, 4], [4, 4, 0]], index=samples, columns=genes)
...
def test_get_min_sample_size():
    ...
    min_sample_size = get_min_sample_size(
        group="group", obs=obs, large_n=10, min_prop=0.7
    )
    assert min_sample_size == 2
    min_sample_size = get_min_sample_size(
        group=obs["group"], obs=obs, large_n=10, min_prop=0.7
    )
    assert min_sample_size == 2
    min_sample_size = get_min_sample_size(
        group=obs["group"].values, obs=obs, large_n=10, min_prop=0.7
    )
    assert min_sample_size == 2
    min_sample_size = get_min_sample_size(
        group=list(obs["group"].values), obs=obs, large_n=10, min_prop=0.7
    )
    assert min_sample_size == 2
...
def test_get_cpm():
    y = np.array([[2, 2, 4, 4, 8], [4, 4, 0, 2, 6]])
    lib_size = np.sum(y, axis=1)

    cpm = get_cpm(y, lib_size=lib_size)
    assert np.all(
        cpm.ravel() == np.array([1e5, 1e5, 2e5, 2e5, 4e5, 25e4, 25e4, 0, 125e3, 375e3])
    )
...
def test_rank_sources_groups():
    ...
    df = rank_sources_groups(acts, groupby="group", reference="rest", method="wilcoxon")
    assert df.shape[0] == 9
    assert np.all((df["statistic"].values > 1.7) == gt_up)
    assert np.all((df["statistic"].values < -1.7) == gt_dw)

    df = rank_sources_groups(acts, groupby="group", reference="rest", method="t-test")
    assert df.shape[0] == 9
    assert np.all((df["statistic"].values > 1.7) == gt_up)
    assert np.all((df["statistic"].values < -1.7) == gt_dw)

    df = rank_sources_groups(
        acts, groupby="group", reference="rest", method="t-test_overestim_var"
    )
    assert df.shape[0] == 9
    assert np.all((df["statistic"].values > 1.7) == gt_up)
    assert np.all((df["statistic"].values < -1.7) == gt_dw)

    df = rank_sources_groups(acts, groupby="group", reference="A", method="t-test")
    assert df.shape[0] == 6

    df = rank_sources_groups(
        acts, groupby="group", reference=["A", "B", "C"], method="t-test"
    )
    assert df.shape[0] == 9
    assert np.all((df["statistic"].values > 1.7) == gt_up)
    assert np.all((df["statistic"].values < -1.7) == gt_dw)

    df = rank_sources_groups(acts, groupby="group", reference="C", method="wilcoxon")
    assert df.shape[0] == 6
    assert np.all(df["group"].values != "C")
    with pytest.raises(ValueError):
        rank_sources_groups(
            acts, groupby="group", reference=["A", "B", "C"], method="asdadssad"
        )
    with pytest.raises(ValueError):
        rank_sources_groups(
            acts, groupby="group", reference="asdadssad", method="t-test"
        )

    acts.X[0, 0] = np.nan
    acts.X[1, 1] = np.inf
    with pytest.raises(ValueError):
        rank_sources_groups(acts, groupby="group", reference="A", method="t-test")
